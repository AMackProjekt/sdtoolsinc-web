...
const NAV = [
  { href: "/portal/news/dashboard",     label: "Dashboard",     icon: LayoutDashboard },
  { href: "/portal/news/articles",      label: "Articles",      icon: Newspaper },
  { href: "/portal/news/announcements", label: "Announcements", icon: Megaphone },
  { href: "/portal/news/media",         label: "Media Library", icon: Image },
  { href: "/portal/news/calendar",      label: "Calendar",      icon: CalendarDays },
  { href: "/portal/news/analytics",     label: "Analytics",     icon: BarChart2 },
  { href: "/portal/news/settings",      label: "Settings",      icon: Settings },
];
...
export default function NewsPortalLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!isLoading && !isAuthenticated && !pathname.endsWith("/auth")) {
      router.replace("/portal/news/auth");
    }
    ...
    <div className="flex min-h-screen bg-bg">
      ...
      >
        ...
        <div className="flex h-16 items-center gap-3 border-b border-rose-900/30 px-5">
          ...
          <span className="text-sm font-bold tracking-tight text-rose-200">News &amp; Media</span>
        </div>
        ...
        {/* Footer */}
        <div className="border-t border-rose-900/30 p-4 space-y-2">
          {user && (
            <div className="flex items-center gap-3 rounded-lg bg-rose-950/40 px-3 py-2.5">
              <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-rose-500/30 text-xs font-bold text-rose-300">
          ...
          <button
            ...
            className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-xs text-slate-400/60 hover:text-rose-300 transition"
          >
            <ArrowLeft size={14} />
            Portal Hub
          </button>
      ...
      <div className="flex flex-1 flex-col lg:pl-64">
        ...
        <InternalChat currentUser={user?.name ?? "News"} role="staff" />
        <MackAI />
      </div>
    </div>
  );
