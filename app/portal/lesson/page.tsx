    ...
    <div className="min-h-screen bg-bg">
      ...
      <header className="border-b border-border bg-panel/50 backdrop-blur-xl sticky top-0 z-40">
        <div className="mx-auto max-w-container px-7 py-4 flex items-center justify-between">
          <Link href={`/portal/course?id=${courseId}`} className="text-brand hover:text-brand2 flex items-center gap-2">
            ← Back to Course
          </Link>
          <button
            onClick={logout}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-3">
            ...
            >
              {prevLesson ? (
                <Link
                  href={`/portal/lesson?courseId=${courseId}&lessonId=${prevLesson.id}`}
              ...
              {nextLesson ? (
                <Link
                  href={`/portal/lesson?courseId=${courseId}&lessonId=${nextLesson.id}`}
                ...
                <Link
                  href={`/portal/course?id=${courseId}`}
                  className="flex-1 px-6 py-3 rounded-lg font-semibold text-sm bg-gradient-to-br from-brand to-brand2 text-[#02131a] hover:shadow-glow transition-all text-center"
                >
                  Back to Course →
                </Link>
                ...
                <div className="space-y-2 max-h-96 overflow-y-auto">
                  {courseLessons.map((l, index) => {
                    ...
                    return (
                      <Link
                        key={l.id}
                        href={`/portal/lesson?courseId=${courseId}&lessonId=${l.id}`}
